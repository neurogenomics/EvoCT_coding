...
gene=$(head -$PBS_ARRAY_INDEX  $repo_dir/data/branchsite_genes.txt | tail -1)
...
if [ -e "${gene}._nobranchlen.branch-site.log" ]; then
...
ete3 evol -t $analysis_dir/$gene/species_tree_nobranchlen.nw --alg $analysis_dir/$gene/${gene}_nostop.fasta -o $temp_dir/$gene/$gene-branch-site --models ${models[*]} --tests ${tests[*]} --cpu 32 --mark ${marks[*]} >> $analysis_dir/$gene/${gene}.branch-site.log
echo "Done with $gene"
